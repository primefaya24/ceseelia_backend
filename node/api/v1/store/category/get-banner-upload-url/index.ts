...
module.exports = function (router: Router): void {
  router.get(
    "/get-banner-upload-url/:storeId/:categoryId/:imageType",
...
function validRequestParams(req: Request): boolean {
  return req.params.imageType === 'png' || req.params.imageType === 'jpg' || req.params.imageType === 'jpeg';
}
...
async function executeRouteCore(req: Request, res: Response): Promise<void> {
  try {
    ...
    const imageAsset = {
      key: `store/${storeId}/category/${categoryId}/images/${ULID.ulid()}.${imageType}`,
      url: "",
    };
